...
You can access your application via **`localhost`**, if you're running the containers directly, or through **`192.168.33.17`** when run on a vm. nginx and mailhog both respond to any hostname, in case you want to add your own hostname on your `/etc/hosts` 

Service|Address outside containers|Address outside VM
------|---------|-----------
Webserver|[localhost:8080](http://localhost:8080)|[192.168.33.17](http://192.168.33.17)
...
------|---------|-----------
php-fpm|dai-usaid-docker-webstarter-php-fpm|9000
MySQL|dai-usaid-docker-webstarter-mysql|3306 (default)
Memcached|dai-usaid-docker-webstarter-memcached|11211 (default)
Redis|dai-usaid-docker-webstarter-redis|6379 (default)
Elasticsearch|dai-usaid-docker-webstarter-elasticsearch|9200 (HTTP default) / 9300 (ES transport default)
...
**Note 2:** if you're using Vagrant, you'll need to ssh into it first
  ...
  * Execute command inside of container: `docker exec -it dai-usaid-docker-webstarter-php-fpm COMMAND` where `COMMAND` is whatever you want to run. For instance, `/bin/bash` to open a console prompt.
